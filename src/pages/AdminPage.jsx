...
function UploadButton({ onUpload }) {
  ...
  return (
    <div>
      <label className="flex items-center gap-3 px-5 py-4 rounded-xl border-2 border-dashed border-sand hover:border-terracotta transition-colors cursor-pointer">
        <Upload size={20} className="text-terracotta" />
        <span className="text-sm text-charcoal/60">{uploading ? 'Uploading...' : 'Click to upload image'}</span>
        <input type="file" accept="image/*" onChange={handleFile} className="hidden" />
      </label>
      {lastUrl && (
        <div className="mt-3 flex items-center gap-3">
          <img src={lastUrl} alt="Uploaded" className="w-16 h-16 rounded-lg object-cover border border-sand" />
          <div className="flex-1">
            <p className="text-xs text-charcoal/40 mb-1">Uploaded URL:</p>
            <input type="text" value={lastUrl} readOnly className="w-full px-3 py-2 rounded-lg border border-sand text-xs bg-sand/10" onClick={(e) => e.target.select()} />
          </div>
        </div>
      )}
...
function Field({ label, value, onChange, type = 'text', placeholder = '', big = false }) {
  ...
  return (
    <div>
      ...
      {big ? (
        <textarea value={local} onChange={(e) => setLocal(e.target.value)} onBlur={() => { if (local !== value) onChange(local) }} rows={3} placeholder={placeholder} className="w-full px-4 py-2.5 rounded-xl border border-sand text-sm focus:outline-none focus:border-terracotta resize-none" />
      ) : (
        <input type={type} value={local} onChange={(e) => setLocal(e.target.value)} onBlur={() => { if (local !== value) onChange(local) }} placeholder={placeholder} className="w-full px-4 py-2.5 rounded-xl border border-sand text-sm focus:outline-none focus:border-terracotta" />
      )}
    </div>
  )
}
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8">
        ...
        {tab === 'images' && (
          <div className="space-y-8">
            {/* Upload Section */}
            <div className="bg-white rounded-2xl p-6 border border-sand/50">
              <h3 className="font-serif text-lg font-semibold text-charcoal mb-4">Upload Image from Computer</h3>
              <p className="text-xs text-charcoal/40 mb-4">
                {apiOnline ? 'Upload an image and get a URL to paste anywhere above.' : 'API not connected — using local mode.'}
              </p>
